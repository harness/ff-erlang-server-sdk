...
get_from_cache_test() ->
  ...
  ?assertEqual(
    {ok, <<"flags/does_exist">>},
    cfclient_cache_repository:get_value({flag, <<"does_exist">>})
  ),
  %% Flag is not found in cache
  meck:expect(cfclient_ets, get, fun (_, <<"flags/does_not_exist">>) -> undefined end),
  ?assertEqual(
    {error, undefined},
    cfclient_cache_repository:get_value({flag, <<"does_not_exist">>})
  ),
  ...
  ?assertEqual(
    ...
    cfclient_cache_repository:get_value({segment, <<"does_exist">>})
  ),
  %% Segment is not found in cache
  meck:expect(cfclient_ets, get, fun (_, <<"segments/does_not_exist">>) -> undefined end),
  ?assertEqual(
    {error, undefined},
    cfclient_cache_repository:get_value({segment, <<"does_not_exist">>})
  ),
